# MONTEO NÚMERO — Especificación Funcional y Arquitectura del MVP

**Estado:** Especificación funcional (Fase 1) y arquitectura documental (Fase 2) **cerradas**. Implementación de código **pendiente**.

## Descripción general

MONTEO NÚMERO es una **PWA educativa** que genera estímulos numéricos para una actividad con **cartas físicas**.
...
> DEC-032 (supercede DEC-023)

```
3 → 2 → 1 → ¡FUERA! → 500 ms → EXPOSURE → número
```
...
**NO** anunciar el número por lector de pantalla durante EXPOSURE. `aria-live` sí para cuenta atrás, ¡FUERA!, instrucciones y resultados.
...
| Compose previsto | servicio/contenedor `monteo-numero`; restart `unless-stopped`; sin volúmenes; sin env del MVP |
...
DESARROLLO LOCAL → BUILD → PRUEBAS → DOCKER BUILD → prueba del mismo artefacto → `docker save` → `monteo-numero-image.tar` → transferencia manual → `docker load` en TerraMaster → Compose/Docker → pruebas en NAS.
...
| Dockerfile / compose / nginx | [ ] PENDIENTE |
