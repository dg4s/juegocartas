# NÚMERO FLASH — Decisiones Arquitectónicas

Registro cronológico de decisiones del proyecto. Cada entrada incluye identificador, fecha, estado y contexto.

> Identidad operativa vigente: **NÚMERO FLASH** (`numero-flash`), DEC-051. Entradas anteriores conservan el nombre histórico **MONTEO NÚMERO** donde corresponda.
...
## DEC-051 — Renombrado definitivo a NÚMERO FLASH

| Campo | Valor |
|-------|-------|
| **Fecha** | 2026-09-06 |
| **Estado** | **APROBADO** |
| **Tema** | Identidad del producto |
| **Decisión** | El producto pasa a denominarse **NÚMERO FLASH** (nombre visible) con slug técnico **`numero-flash`**. |
| **Nombre visible** | NÚMERO FLASH |
| **Slug / directorio** | `numero-flash` |
| **Ruta local** | `C:\Dev\juegocartas\numero-flash` |
| **Servicio Docker** | `numero-flash` |
| **Contenedor Docker** | `numero-flash` |
| **Imagen Docker** | `numero-flash:latest` |
| **Archivo TAR** | `numero-flash-image.tar` |
| **Host** | **8095** (sin cambio, DEC-011) |
| **Puerto contenedor** | **8080** (sin cambio, DEC-045) |
| **Mapeo** | `8095:8080` |
| **Supercede operativamente** | Campos operativos de DEC-001 (nombre/directorio), DEC-007 (nombre TAR), DEC-046 (servicio/contenedor/imagen compose) |
| **Conserva como historia** | DEC-001, DEC-007, DEC-046 — entradas originales no se reescriben |
| **Rediseño visual** | Interfaz de juego educativo con cartas; carta central como elemento protagonista; pantallas inicio, juego, verificación y resultados rediseñadas |
| **Mecánica** | Sin cambios en reglas, estados, timers, generador 648, 5 rondas, cancelación, audio, PWA, accesibilidad |
| **Infraestructura** | NAS, Caddy, DuckDNS, origin GitHub — sin cambios |

---

## DEC-052 — Rótulo GO en la señal de arranque

| Campo | Valor |
|-------|-------|
| **Fecha** | 2026-09-06 |
| **Estado** | **APROBADO** |
| **Tema** | Texto visible y accesibilidad en GO_SIGNAL |
| **Supercede** | Rótulo «¡FUERA!» en la UI y en `aria-live` |
| **Secuencia vigente** | **3 → 2 → 1 → GO → 500 ms → EXPOSURE → número** |
| **Estado técnico** | Sigue llamándose **GO_SIGNAL** (sin renombrar fase, acción ni tests de reducer) |
| **Duración** | 500 ms sin cambio (DEC-032) |
| **Audio** | Señal distintiva breve en GO_SIGNAL; sin sonido extra al aparecer el número (DEC-019, DEC-035) |
| **Histórico** | DEC-032 conserva el texto «¡FUERA!» como evidencia; no reescribir DEC-019 ni DEC-032 |
